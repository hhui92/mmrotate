...
def add_parser(parser):
    ...
    parser.add_argument('--img-dirs', nargs='+', type=str, default=None,
        help='images dirs, must give a value')
    parser.add_argument('--ann-dirs', nargs='+', type=str, default=None,
    ...
    parser.add_argument('--gaps', nargs='+', type=int, default=[200],
    ...
    parser.add_argument('--padding-value', nargs='+', type=int,
        default=[0], help='padding value, 1 or channel number')
...
def parse_args():
    ...
    if args.base_json is not None:
        ...
        for action in parser._actions:
            if action.dest not in prior_config or not hasattr(action, 'default'):
                continue
    ...
    assert args.iof_thr >= 0 and args.iof_thr <= 1
    assert not osp.exists(args.save_dir), f'{osp.join(args.save_dir)} already exists'
    return args
...
def get_sliding_window(info, sizes, gaps, img_rate_thr):
    ...
    img_areas = (img_in_wins[:, 2] - img_in_wins[:, 0]) * (img_in_wins[:, 3] - img_in_wins[:, 1])
...
def crop_and_save_img(info, windows, window_anns, img_dir, no_padding,
    ...
    for i in range(windows.shape[0]):
        ...
        patch_info['id'] = info['id'] + '__' + str(x_stop - x_start) + '__' + str(x_start) + '___' + str(y_start)
...
def load_dota(img_dir, ann_dir=None, nproc=10):
    ...
    end_time = time.time()
    print(f'Finishing loading DOTA, get {len(contents)} iamges,', f'using {end_time - start_time:.3f}s.')
...
def _load_dota_txt(txtfile):
    ...
    diffs = np.array(diffs, dtype=np.int64) if diffs else np.zeros((0,), dtype=np.int64)
...
def main():
    ...
    padding_value = args.padding_value[0] if len(args.padding_value) == 1 else args.padding_value
    sizes, gaps = [], []
